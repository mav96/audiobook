...
class DownloadManager():
    ...
    def process_events(self):
        ...
        for (b, h) in self.handles.items():
            ...
            if s.is_seeding:
                ...
                print('the book with uid %s: %.2f%% complete'
                      '(down: %.1f kb/s up: %.1f kB/s peers: %d) %s' % (b,
                                                                        s.progress * 100,
    ...
    def remove_torrent(self, book_uid):
        if book_uid in self.handles:
            ...
            self.handles[book_uid]['ch'].basic_ack(delivery_tag=self.handles[book_uid]['metod'].delivery_tag)
